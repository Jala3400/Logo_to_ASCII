...
-   `-d, --dict <DICTS>...`: Use a built-in character set. Overrides `--chars` but not `--add-chars` and `--except`. You can specify multiple times or as a comma-separated list [possible values: all, symbols, blocks, blocks_all, box, box_all, box_double, box_double_all, nerd, math, numbers, letters]
...
-   `-b, --borders <BORDER_CRITERIA>`: Separates colors (change thickness with `-k`) [possible values: color, brightness, all]
